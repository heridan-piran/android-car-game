...
class Vehicle extends Unit
{
	...
	public Vehicle(boolean col, boolean movs, double px, double py, double w, double h, double ang, 
			double topsp, double accel, double sdrate, double brkrate, double turn,double spd , Drawable img)
	...
	public void updateSpeed(float axisrotation, int maxrotate)
	{
		if(Math.abs(axisrotation) > 0)
		{
			...
			else if (axisrotation < -40) //reverse
			{
				...
				if (speed < -1 * (topspeed / 5)) {speed = -1 * (topspeed / 5);}
			}
